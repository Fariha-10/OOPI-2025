//Name;Fariha Noshin
//ID:100988168
//Date:12-02-2025
//Description: Class exercise 2

//passing by value:
//When a primitive data type like an int or a double is passed into a function as an argument,
//the function treats that parameter as a “copy” and the original value does not change.
//This can be called passing by value.

//passing by reference:
//Data types like Strings, arrays, and objects are stored
// as a reference to their location in memory, so a function operating on them can modify them.
//This can be called passing by reference.


import java.util.Arrays;
import java.util.Scanner;

public class Main {
    public static void main(String[] args) {

        // Pass by Value
        int x = 5;
        System.out.println("Before calling increment: x = " + x);
        increment(x);
        System.out.println("After calling increment: x = " + x);

        // Pass by Reference
        int[] arr = {2, 4, 6, 8, 10};
        System.out.println("\nBefore calling modifyArray: " + Arrays.toString(arr));
        modifyArray(arr);
        System.out.println("After calling modifyArray: " + Arrays.toString(arr));

        // Function  Swap Values
        Scanner scanner = new Scanner(System.in);
        int[] numbers = new int[5];

        System.out.println("\nEnter 5 numbers: ");
        for (int i = 0; i < 5; i++) {
            numbers[i] = scanner.nextInt();
        }


        System.out.println("Enter two indices to swap: ");
        int index1 = scanner.nextInt();
        int index2 = scanner.nextInt();

        // Swap values and display the modified array
        swapValues(numbers, index1, index2);
        System.out.println("Array after swapping: " + Arrays.toString(numbers));

    }

    // Pass by Value Method
    static void increment(int num) {
        num = num + 10;
        System.out.println("Inside increment method: x = " + num);
    }

    // Pass by Reference Method
    static void modifyArray(int[] arr) {
        for (int i = 0; i < arr.length; i++) {
            arr[i] += 10;
        }
        System.out.println("Inside modifyArray: " + Arrays.toString(arr));
    }

    // Function to Swap Values
    static void swapValues(int[] arr, int i, int j) {
        if (i >= 0 && i < arr.length && j >= 0 && j < arr.length) {
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
            System.out.println("Swapping successful!");
        }
        else {
            System.out.println("Invalid indices. Please enter values between 0 and " + (arr.length - 1));}

        }
    }